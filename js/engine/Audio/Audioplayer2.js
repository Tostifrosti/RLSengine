...
AudioPlayer.prototype.play = function(name, options) {
	...
	if(typeof options.start === 'undefined' || (options.start < 0.00 || options.start >= this.songs[name].duration)) { 
		options.start = 0.00;
	}
...
AudioPlayer.prototype.stop = function(name) {
	...
	song.stop();
	this.isPlaying.splice(song.name, i);
};
